...
namespace AlexPilotti.FTPS.Client
{
    ...
    public sealed class FTPSClient : IDisposable
    {
        ...
        async public Task<string> Connect(string hostname)
        {
            return await Connect(hostname, defaultSSLSupportMode);
        }
        ...
        /// <param name="hostname"></param>
        /// <returns>The text of the \"welcome message\" sent by the server.</returns>
        async public Task<string> Connect(string hostname, ESSLSupportMode sslSupportMode)
        {
            return await Connect(hostname, null, sslSupportMode);
        }

        async public Task<string> Connect(string hostname, NetworkCredential credential)
        {
            return await Connect(hostname, credential, defaultSSLSupportMode);
        }

        async public Task<string> Connect(string hostname, NetworkCredential credential, ESSLSupportMode sslSupportMode)
        {
            return await Connect(hostname, credential, sslSupportMode, null);
        }
                            ...
                            RemoteCertificateValidationCallback userValidateServerCertificate)
        {
            ...
            return await Connect(hostname, port, credential, sslSupportMode, userValidateServerCertificate, null, 0, 0, 0, null);
        }
        ...
        /// <returns>The text of the \"welcome message\" sent by the server.</returns>
        async public Task<string> Connect(string hostname, int port, NetworkCredential credential, ESSLSupportMode sslSupportMode, 
                            ...
                            int? timeout)
        {
            return await Connect(hostname, port, credential, sslSupportMode, userValidateServerCertificate, x509ClientCert,
                            ...
                            int? timeout, bool useCtrlEndPointAddressForData, EDataConnectionMode dataConnectionMode)
        {
            ...
            await SetupCtrlConnection(hostname, port, Encoding.ASCII);
        ...
        public Task<ulong> PutFileAsync(string localFileName, string remoteFileName)
        {
            return PutFileAsync(localFileName, remoteFileName, null);
        }
        ...
        async public Task<ulong> PutFileAsync(string localFileName, string remoteFileName, FileTransferCallback transferCallback)
        {
            using (Stream s = PutFile(remoteFileName))
                return await SendFileAsync(localFileName, remoteFileName, s, transferCallback);
        }
        ...
        ulong SendFile(string localFileName, string remoteFileName, Stream s, FileTransferCallback transferCallback)
        {
            ulong totalBytes = 0;
        ...
        async Task<ulong> SendFileAsync(string localFileName, string remoteFileName, Stream s, FileTransferCallback transferCallback)
        {
            ulong totalBytes = 0;
            ...
            using (FileStream fs = File.OpenRead(localFileName))
            {
                ...
                do
                {
                    ...
                    n = await fs.ReadAsync(buf, 0, buf.Length);
                    if (n > 0)
                    {
                        await s.WriteAsync(buf, 0, n);
                        totalBytes += (ulong)n;
                    }
                }
                while (n > 0);
        ...
        async Task SetupCtrlConnection(string hostname, int port, Encoding textEncoding)
        {
            CloseCtrlConnection();

            ctrlClient = new TcpClient();

            await ctrlClient.ConnectAsync(hostname, port);
